...
推論・提出コードを変更する前に、`docs/engineering_notes.md` の Kaggle Submission を読むこと。

### 実提出失敗時の絶対ルール

Kaggle Notebookの手動実行と、コンペ提出後のhidden test再実行を混同しないこと。

- **Notebook手動実行**: ログ・print出力を確認できるが、使われるのは公開されたsample/pseudo testであり、本番のhidden testではない。データ量、値の分布、エッジケースなどが異なり得るため、手動実行の成功は実提出の成功を保証しない
- **コンペへの実提出**: 非公開のhidden testで再実行される。この実行のログ・print出力は一切確認できず、成功/失敗などの状態だけが観測可能

実提出がERRORになった場合は以下を厳守すること。

- ログやprintを追加してhidden testで原因を調べる提案をしてはならない。追加しても確認できない
- 例外を握りつぶす、定数予測へ切り替える、sample submissionを流用する、失敗行をスキップする、代替モデルを使うなど、ユーザーが意図しないフォールバックを実装してはならない
- 想定外の状態はfail fastにし、エラーをスコア低下へ変換しない
- 原因はローカルまたはNotebook手動実行で再現する。hidden testとの差を想定した入力規模・値・欠損・エッジケース・時間・メモリ条件も検証する
- 再現できない場合は原因を断定せず、根拠のない修正や「提出を通すためだけ」の処理を加えない
