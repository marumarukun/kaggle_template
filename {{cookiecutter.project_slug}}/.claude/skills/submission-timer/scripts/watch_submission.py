...
def main():
    ...
    # 既にPENDINGのものがあれば追跡対象にする
    tracking: dict[int, float] = {}  # ref -> 検知時刻
    for s in initial_subs:
        if "PENDING" in s["status"]:
            tracking[s["ref"]] = time.time()
            log(f"既存のPENDING提出を検知: {s['description']} (ref={s['ref']})")
    ...
    def handle_sigint(signum, frame):
        print(flush=True)
        if tracking:
            elapsed = time.time() - min(tracking.values())
    ...
    while True:
        ...
        for s in current_subs:
            if s["ref"] not in initial_refs and s["ref"] not in tracking:
                ...
                log(f"新しい提出を検知! {s['description']} (ref={s['ref']})")

        # 追跡中の提出のステータスチェック
        completed = []
        for ref, start_time in tracking.items():
