...
1. **スナップショット取得**: 現在の提出一覧を取得し、既存の提出IDを記録。既にPENDING状態の提出も追跡対象に含める
2. **待機**: 新しい提出（スナップショットに存在しない提出）が現れるまでポーリング
3. **計測**: 起点は Kaggle API の `submission.date`（実際の提出時刻、UTC）を使用。スクリプト起動時刻やポーリング検知時刻ではなく、提出そのものの時刻を採用する
4. **完了検知**: ステータスがCOMPLETEまたはERRORに変わったら結果を報告
5. **報告内容**:
   - 提出時刻・完了時刻（ローカルタイムゾーンで表示）
   - 経過時間（提出時刻 → 完了時刻）
...
[12:00:01] 新しい提出を待機中...
[12:01:01] 新しい提出を検知! Version 54 (ref=12345678, 提出時刻=2026-05-18 12:01:00)
[12:02:01] PENDING... Version 54 (経過: 1分1秒)
...
==================================================
  COMPLETE!
  提出: Version 54
  提出時刻: 2026-05-18 12:01:00
  完了時刻: 2026-05-18 12:15:32
  経過時間: 14分32秒
  スコア: 0.928
