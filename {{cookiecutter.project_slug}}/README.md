...
> **⚠️ 重要な制約: model_sources には最新バージョンのみ指定可能**
>
> Kaggle サーバーの仕様により、`model_sources` に指定できるのは**その Model Instance の最新バージョンのみ**です。
> 古いバージョン（例: version 1〜3 がある状態で `001/1` を指定）を指定した場合、`kaggle kernels push` はエラーなく成功しますが、
> **モデルがノートブックに紐づかず、実行時に FileNotFoundError になります**。
> （Kaggle CLI 2.0.0 ではサーバーが返す `invalidModelSources` エラーが表示されないバグがあります）
>
> **回避策**: 複数バージョンを順番に提出したい場合は、以下の手順で進めてください：
> 1. バージョン A をアップロード → 提出（この時点では A が最新なので OK）
> 2. バージョン B をアップロード → 提出（この時点では B が最新なので OK）
>
> つまり「1つアップロード → 提出 → 次をアップロード → 提出」の順序で進める必要があります。
